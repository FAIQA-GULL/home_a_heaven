...
if(isset($_GET["action"]))
{
	if($_GET["action"] == "pending")
	{
        ...
        $confirm_order ="UPDATE order_info , cart SET order_info.O_status='pending' , cart.status = 'pending' WHERE order_info.order_id = '$order_id' AND cart.order_id = '$order_id' ";
        print_r($confirm_order);

                $query = mysqli_query($conn , $confirm_order) ;
                echo '<script>alert("Order is Pending Now!")</script>';
...
if(isset($_GET["action"]))
{
	if($_GET["action"] == "in-process")
	{
        ...
        $confirm_order ="UPDATE order_info , cart SET order_info.O_status='in-process' , cart.status = 'in-process' WHERE order_info.order_id = '$order_id' AND cart.order_id = '$order_id' ";
        print_r($confirm_order);

                $query = mysqli_query($conn , $confirm_order) ;
                echo '<script>alert("Order is In-process Now!")</script>';
				// echo '<script>window.location="admin.php"</script>';
...
if(isset($_GET["action"]))
{
	if($_GET["action"] == "completed")
	{
        ...
        $confirm_order ="UPDATE order_info , cart SET order_info.O_status='completed' , cart.status = 'completed' WHERE order_info.order_id = '$order_id' AND cart.order_id = '$order_id' ";
        print_r($confirm_order);
...
<div class="container">
	<br><br>
	<!-- <h1>Welcome Admin</h1>	 -->
	<center><h2 class="head">Confirmed Order</h2></center>
	<br>
	...
	<table class="table">
		...
		$sql="SELECT * FROM order_info 
		  WHERE cdate = CURDATE() AND O_status = 'confirmed'";
		  // print_r($sql);
		...
		<tbody>
			<tr>
				...
				<td>
					<a href="confirmed_orders.php?action=pending&order_id=<?php echo $row['order_id'];?>" class="btn btn-primary">Pending</a> 
					||
					<a href="confirmed_orders.php?action=in-process&order_id=<?php echo $row['order_id'];?>" class="btn btn-warning">In-process</a>
					||
					<a href="confirmed_orders.php?action=completed&order_id=<?php echo $row['order_id'];?>" class="btn btn-success">Completed</a>
					||
					<a href="confirmed_orders.php?action=cancel_order&order_id=<?php echo $row['order_id'];?>" class="btn btn-danger">Cancel</a>
